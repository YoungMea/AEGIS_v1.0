...
export function EagleEyeSection() {
  const { t } = useI18n();
  ...
  const startScan = useCallback(async () => {
    ...
    setScanning(true);
    try {
      const res = await fetch("/api/eagleeye/scan", {
        method: "POST",
    ...
    try {
      const res = await fetch("/api/dossiers/append", {
        ...
        body: JSON.stringify({
          ...
          tags: ["eagleeye", report.mode, report.query.slice(0, 32)],
        }),
      });
      if (!res.ok) {
        const txt = await res.text();
        toast.push({
          type: "error",
          title: t.eagleEye?.appendError ?? "Could not append",
      ...
      toast.push({
        type: "success",
        title: t.eagleEye?.appendOk ?? "Filed to dossier",
        message:
          (t.eagleEye?.appendSummary ?? "Added") +
    ...
    <div className="pt-6 pb-12">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-5">
        ...
        <div className="flex flex-wrap gap-3">
          <Stat label={t.eagleEye?.statMatches ?? "MATCHES"} value={stats.found} />
          <Stat
            label={t.eagleEye?.statArchive ?? "ARCHIVED"}
            value={stats.archive}
            accent
          />
          <Stat label={t.eagleEye?.statLinks ?? "LINKS"} value={stats.links} />
        </div>
      </div>
      ...
      <div className="surface-strong rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        <div className="flex items-center surface px-1 h-10 shrink-0">
          <ModeBtn
            ...
            icon={<AtSign size={12} />}
            label={t.eagleEye?.modeUsername ?? "Username"}
          />
          <ModeBtn
            active={mode === "email"}
            onClick={() => setMode("email")}
            icon={<Mail size={12} />}
            label={t.eagleEye?.modeEmail ?? "Email"}
          />
          <ModeBtn
            active={mode === "phone"}
            onClick={() => setMode("phone")}
            icon={<Phone size={12} />}
            label={t.eagleEye?.modePhone ?? "Phone"}
          />
        </div>
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={
              mode === "username"
                ? t.eagleEye?.placeholderUsername ?? "agent_codename"
                : mode === "phone"
                  ? t.eagleEye?.placeholderPhone ?? "998901234567"
                  : t.eagleEye?.placeholderEmail ?? "target@domain.com"
            }
            className={cn(
        ...
        >
          ...
          {scanning
            ? t.eagleEye?.scanning ?? "Scanning"
            : t.eagleEye?.runScan ?? "Run scan"}
        </button>
        ...
        {report && (
          ...
          >
            ...
            {t.eagleEye?.saveToDossier ?? "Save to dossier"}
          </button>
        )}
      </div>
  ...
  return (
    <div className={cn("rounded-xl border p-5", tone)}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display tracking-[0.18em] uppercase text-[13px] text-white">
            {t.eagleEye?.aiTitle ?? "AI Identity Narrative"}
          </span>
        </div>
        <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-white/40">
          {t.eagleEye?.confidence ?? "CONFIDENCE"} · {conf}/100
        </div>
      </div>
      ...
      <div className="mt-3 font-mono text-[9px] uppercase tracking-[0.22em] text-white/30">
        {t.eagleEye?.via ?? "VIA"} · {ai.provider.toUpperCase()}
      </div>
    </div>
  );
}
    ...
    <div className="rounded-xl border border-white/[0.08] bg-gradient-to-br from-fuchsia-500/10 to-amber-500/[0.03] p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {ig.isPrivate && (
              <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-white/55 inline-flex items-center gap-1">
                <Lock size={9} /> {t.eagleEye?.private ?? "PRIVATE"}
              </span>
            )}
          </div>
          ...
          >
            <ExternalLink size={10} /> {t.eagleEye?.openProfile ?? "Open profile"}
          </a>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="surface-strong rounded-xl p-4">
      <div className="font-display tracking-[0.18em] uppercase text-[13px] text-white mb-3 flex items-center gap-2">
        ...
        {t.eagleEye?.probeTitle ?? "Cross-platform probe"}
      </div>
  ...
  return (
    <div className="surface-strong rounded-xl p-4">
      <div className="font-display tracking-[0.18em] uppercase text-[13px] text-white mb-3 flex items-center gap-2">
        ...
        {t.eagleEye?.archiveTitle ?? "Wayback timeline"}
      </div>
      {archive.length === 0 ? (
        <div className="text-[11px] text-white/40">
          {t.eagleEye?.archiveEmpty ??
  ...
  return (
    <div className="surface-strong rounded-xl p-4">
      <div className="font-display tracking-[0.18em] uppercase text-[13px] text-white mb-3 flex items-center gap-2">
        ...
        {t.eagleEye?.bundleTitle ?? "Collected links"}
      </div>
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="surface px-3 py-2">
          <div className="label-mono flex items-center gap-1">
            <Link2 size={9} /> {t.eagleEye?.linkCount ?? "LINKS"}
          </div>
        ...
        <div className="surface px-3 py-2">
          <div className="label-mono flex items-center gap-1">
            <ImageIcon size={9} /> {t.eagleEye?.imageCount ?? "IMAGES"}
          </div>
          <div className="font-display text-[15px] text-white">
      ...
      {report.links.length === 0 && (
        <div className="text-[11px] text-white/40">
          {t.eagleEye?.bundleEmpty ?? "Run a scan to collect links."}
        </div>
      )}
      {report.links.length > 0 && (
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="font-display tracking-[0.18em] uppercase text-[14px] text-white">
            {t.eagleEye?.pickDossier ?? "Pick a dossier"}
          </div>
          <button
            onClick={onClose}
